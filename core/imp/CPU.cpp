...
uint8_t CPU::status_as_int(){ 
    VNES_LOG::LOG(VNES_LOG::WARN, "Check that I'm implemented right (is bit 5 correct?)!");
    uint8_t status = 0b00010000; // bit 5 is always 1
    if(carry_f)               status |= 0b10000000;
    if(zero_f)                status |= 0b01000000;
    if(interrupt_disable_f)   status |= 0b00100000;
    if(b_flag_f)              status |= 0b00001000;
    if(decimal_f)             status |= 0b00000100;
    if(overflow_f)            status |= 0b00000010;
    if(negative_f)            status |= 0b00000001;
    return status;
}
